  ...
  async fetch(request: Request, env: unknown, ctx: unknown) {
    try {
      ...
      if (url.pathname === "/api/market-data/status") {
        ...
        return new Response(JSON.stringify(serverMarketData.getStatus()), {
          status: 200,
          headers: { "content-type": "application/json", "cache-control": "no-cache" },
        });
      }
      if (url.pathname === "/api/market-data/ticks") {
        const { serverMarketData } = await import("./services/server-market-data");
        return new Response(JSON.stringify(serverMarketData.getSnapshots()), {
          status: 200,
          headers: { "content-type": "application/json", "cache-control": "no-cache" },
        });
      }
